...
class TrafslyAdapter:
    ...
    async def offers(self, user: OpContext, limit: int) -> list[PartnerOffer]:
        ...
        for item in sponsors:
            ...
            if not url or not isinstance(ads_id, int):
                continue
            title = str(item.get("title") or "").strip() or title_from_link(url, "Задание Trafsly")
            result.append(
                PartnerOffer(
                    external_id=str(ads_id),
                    title=title,
                    url=url,
                    kind=str(item.get("resource_type") or "channel"),
                    provider=self.name,
                    done=str(item.get("status") or "unsubscribed").lower() not in PENDING_STATUSES,
                    meta={"ads_id": ads_id},
                )
            )
        return result
    ...
    async def confirm(self, user: OpContext, offer: PartnerOffer) -> bool | None:
        ads_id = offer.meta.get("ads_id")
        if not isinstance(ads_id, int):
            if not offer.external_id.isdigit():
                return None
            ads_id = int(offer.external_id)
        try:
            status, data = await self._post(
                "confirm-subscription", {"user_id": user.user_id, "ads_id": ads_id}
            )
        except Exception:
            return None
        if status >= 400 or data.get("detail"):
