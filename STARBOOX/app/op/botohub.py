...
class BotoHubAdapter:
    ...
    async def _tasks(self, user: OpContext, limit: int) -> list[dict[str, Any]] | None:
        body: dict[str, Any] = {"chat_id": user.user_id, "max_op": max(1, limit)}
        try:
            status, payload = await post_json(
                self._url(),
                json=body,
    ...
    async def offers(self, user: OpContext, limit: int) -> list[PartnerOffer]:
        if not self._ready():
            return []
        tasks = await self._tasks(user, limit)
        if not tasks:
            return []
        ...
        for item in tasks:
            ...
            offers.append(
                PartnerOffer(
                    external_id=str(item.get("resource_id") or url),
                    title=title_from_link(url, "Задание BotoHub"),
                    url=url,
                    kind=kind,
                    provider=self.name,
                    done=item.get("completed") is True,
                )
            )
        return offers

    async def confirm(self, user: OpContext, offer: PartnerOffer) -> bool | None:
        tasks = await self._tasks(user, 20)
        if tasks is None:
            return None
        for item in tasks:
            url = str(item.get("url") or item.get("link") or "")
            if str(item.get("resource_id") or url) != offer.external_id:
                continue
            return item.get("completed") is True
        # BotoHub drops completed resources from the list after a while.
        return True
