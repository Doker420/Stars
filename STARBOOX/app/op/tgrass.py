...
from app.op.base import OpContext, OpResult, PartnerOffer, Sponsor, title_from_link
...
class TGrassAdapter:
    ...
    async def _fetch_offers(self, user: OpContext, limit: int) -> list[dict[str, Any]] | None:
        body: dict[str, Any] = {
            "tg_user_id": user.user_id,
            "is_premium": bool(user.is_premium),
            "lang": user.language_code or "ru",
            "offers_limit": max(1, limit),
        }
        if user.username:
        ...
        if status >= 400 or data.get("detail") or data.get("error"):
            return None
        if str(data.get("status") or "").lower() == "no_offers":
            return []
        offers = data.get("offers")
        return [item for item in offers if isinstance(item, dict)] if isinstance(offers, list) else []

    async def offers(self, user: OpContext, limit: int) -> list[PartnerOffer]:
        if not self._has_api():
            return []
        rows = await self._fetch_offers(user, limit)
        if not rows:
            return []
        result: list[PartnerOffer] = []
        for item in rows:
            ...
            title = str(item.get("name") or "").strip() or title_from_link(
                url, OFFER_TITLES.get(kind, "Задание TGrass")
            )
            result.append(
                PartnerOffer(
                    external_id=str(item.get("offer_id") or item.get("channel_id") or url),
                    title=title,
                    url=url,
                    kind=kind,
                    provider=self.name,
                    done=item.get("subscribed") is True,
                )
            )
        return result

    async def confirm(self, user: OpContext, offer: PartnerOffer) -> bool | None:
        rows = await self._fetch_offers(user, 20)
        if rows is None:
            return None
        for item in rows:
            url = str(item.get("link") or "")
            external = str(item.get("offer_id") or item.get("channel_id") or url)
            if external != offer.external_id:
                continue
            return item.get("subscribed") is True
        # Offer is gone from the list — TGrass rotated it, treat as completed.
        return True
