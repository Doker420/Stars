...
from app.services.onboarding import intro_active, intro_limit

_SKIP_PREFIXES = (
    "op:",
    "admin:",
    "noop",
    # Opting out of reminders must work even for a user held at the gate:
    # the alternative is that they block the bot instead.
    "menu:digest:",
)
...
class OpGateMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        if _op_fresh(user, settings.op_cache_sec):
            return await handler(event, data)
        if intro_active(user, settings):
            # Soft-onboarding window: let the newcomer explore the bot first.
            return await handler(event, data)
        ...
        result = await self._gate.enforce(
            ctx, session, settings=settings, max_sponsors=intro_limit(user, settings)
        )
        if result.allowed:
