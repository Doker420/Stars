...
def campaigns_home(queue: Sequence[Campaign]) -> InlineKeyboardMarkup:
    rows = [
        [
            button(
                f"{CAMPAIGN_KIND_EMOJI.get(c.kind, '🔗')} #{c.id} {c.title[:22]}",
                f"admin:camp:{c.id}",
            )
        ]
        for c in queue[:8]
    ]
    rows.append([button("🔄 Обновить", "admin:camp"), button(BACK, "admin:home")])
    return markup(*rows)


def campaign_admin_card(campaign: Campaign) -> InlineKeyboardMarkup:
    rows = []
    if campaign.status == CampaignStatus.MODERATION.value:
        rows.append(
            [
                button("✅ Одобрить", f"admin:camp:{campaign.id}:ok"),
                button("🔴 Отклонить", f"admin:camp:{campaign.id}:no"),
            ]
        )
    elif campaign.status in {CampaignStatus.ACTIVE.value, CampaignStatus.PAUSED.value}:
        rows.append([button("🔴 Снять и вернуть Stars", f"admin:camp:{campaign.id}:no")])
    if campaign.telegram_charge_id and campaign.refunded_at is None:
        rows.append([button("↩️ Вернуть оплату", f"admin:camp:{campaign.id}:refund")])
    rows.append([button("👤 Владелец", f"admin:u:{campaign.owner_id}")])
    rows.append([button("‹ Очередь", "admin:camp"), button(BACK, "admin:home")])
    return markup(*rows)


def campaign_moderation(campaign: Campaign) -> InlineKeyboardMarkup:
    return markup(
        [
            button("✅ Одобрить", f"admin:camp:{campaign.id}:ok"),
            button("🔴 Отклонить", f"admin:camp:{campaign.id}:no"),
        ],
        [button("🔎 Открыть", f"admin:camp:{campaign.id}")],
    )
