...
def home(version: str, pending: int, running_broadcast: bool, maintenance: bool) -> str:
    ...
    status = ("\n".join(flags) + "\n\n") if flags else ""
    return (
        f"🛠 <b>Админ-панель {BRAND}</b> <i>v{h(version)}</i>\n\n"
        f"{status}"
...
# --- paid promotion (moderation) --------------------------------------------------------


def campaigns_home(
    queue: Sequence[Campaign],
    stats: dict[str, int],
    payouts: Sequence[tuple[str, int, int]],
) -> str:
    lines = [
        "📣 <b>Продвижение и партнёрские задания</b>",
        "",
        f"Кампаний всего: <b>{stats['total']}</b> · активных: {stats['active']}",
        f"На модерации: <b>{stats['moderation']}</b>",
        f"Выполнений: {stats['completions']} · выручка: <b>{stats['revenue_xtr']} XTR</b>",
    ]
    if payouts:
        lines += ["", "🤝 <b>Партнёрские задания</b> (выполнено / выплачено):"]
        for provider, count, total in payouts:
            title = PROVIDER_TITLES.get(provider, provider)
            lines.append(f"   • {h(title)}: {count} / {total} {STAR}")
    if queue:
        lines += ["", f"⏳ Ждут проверки: {len(queue)} — выберите кампанию ниже."]
    else:
        lines += ["", "✅ Очередь модерации пуста."]
    return "\n".join(lines)


def campaign_admin_card(campaign: Campaign, owner: User | None) -> str:
    emoji = CAMPAIGN_KIND_EMOJI.get(campaign.kind, "🔗")
    status = CAMPAIGN_STATUS_LABELS.get(campaign.status, campaign.status)
    who = (
        f"{h(owner.display_name)} (<code>{owner.id}</code>) {mention(owner.id, '↗')}"
        if owner
        else str(campaign.owner_id)
    )
    lines = [
        ...
        f"Ссылка: {h(campaign.url)}",
    ]
    if campaign.check_chat:
        lines.append(f"Проверка подписки: <code>{h(campaign.check_chat)}</code>")
    lines += [
        f"Награда: {campaign.reward} {STAR} × {campaign.target_count} "
        f"(выполнено {campaign.done_count})",
        f"Оплата: <b>{campaign.xtr_price} XTR</b>"
        + (" · возвращена" if campaign.refunded_at else ""),
        f"Создана: {fmt_dt(campaign.created_at)}",
    ]
    if campaign.description:
        lines += ["", h(campaign.description)]
    if campaign.moderation_note:
        lines += ["", f"💬 Причина отклонения: {h(campaign.moderation_note)}"]
    return "\n".join(lines)


def campaign_moderation_alert(campaign: Campaign, owner: User | None) -> str:
    who = f"{h(owner.display_name)} (<code>{owner.id}</code>)" if owner else str(campaign.owner_id)
    emoji = CAMPAIGN_KIND_EMOJI.get(campaign.kind, "🔗")
    return (
        f"🔔 <b>Новая кампания на модерации #{campaign.id}</b>\n\n"
        f"{emoji} {h(campaign.title)}\n"
        f"Заказчик: {who}\n"
        f"Формат: {h(CAMPAIGN_KIND_LABELS.get(campaign.kind, campaign.kind))}\n"
        f"Ссылка: {h(campaign.url)}\n"
        f"Объём: {campaign.target_count} × {campaign.reward} {STAR} "
        f"· оплачено {campaign.xtr_price} XTR"
    )


def campaign_reject_prompt(campaign_id: int) -> str:
    return (
        f"Причина отклонения кампании #{campaign_id}. Заказчик увидит её в уведомлении, "
        "а оплаченные Stars будут возвращены автоматически. Отправьте текст."
    )


def campaign_rejected(campaign: Campaign, refunded: bool) -> str:
    tail = (
        "↩️ Оплата возвращена заказчику."
        if refunded
        else "⚠️ Автовозврат не прошёл — верните оплату вручную через «Платежи»."
    )
    return f"🔴 Кампания #{campaign.id} отклонена.\n\n{tail}"
