...
from app.db.models import Broadcast, Campaign, User, Withdrawal
...
class Notifier:
    ...
    async def _handle(self, event: DomainEvent) -> None:
        ...
        match event.name:
            ...
            case "partner_task_completed":
                await self._send(
                    payload["user_id"],
                    texts.notify_partner_task(
                        payload["title"], payload["amount"], payload.get("provider", "")
                    ),
                )
            case "campaign_status":
                await self._campaign_status(payload)
            case "campaign_moderation":
                await self._campaign_moderation(payload["campaign_id"])
    ...
    async def _campaign_status(self, payload: dict[str, Any]) -> None:
        async with self._factory() as session:
            campaign = await session.get(Campaign, payload["campaign_id"])
            if campaign is None:
                return
            text = texts.notify_campaign_status(campaign, payload["status"], payload.get("note", ""))
        await self._send(payload["owner_id"], text)

    async def _campaign_moderation(self, campaign_id: int) -> None:
        """Ping admins about a freshly paid campaign waiting for review."""
        async with self._factory() as session:
            campaign = await session.get(Campaign, campaign_id)
            if campaign is None:
                return
            owner = await session.get(User, campaign.owner_id)
            text = admin_texts.campaign_moderation_alert(campaign, owner)
            markup = admin_keyboards.campaign_moderation(campaign)
        await self._send(
            campaign.owner_id,
            "📣 Оплата получена! Кампания отправлена на модерацию — "
            "мы сообщим, как только её проверят.",
        )
        for admin_id in self._access.all_admin_ids():
            await self._send(admin_id, text, reply_markup=markup)
