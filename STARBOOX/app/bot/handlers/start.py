...
from app.services.onboarding import begin_intro, intro_limit
...
async def cmd_start(
    ...
    if not is_admin:
        result = await op_gate.enforce(
            _ctx(db_user, message.chat.id, bot),
            session,
            settings=settings,
            max_sponsors=intro_limit(db_user, settings),
        )
        if not result.allowed:
            # First contact: show a short sponsor block, then let them in anyway.
            # The remaining sponsors come back later as paid partner tasks.
            if begin_intro(db_user, settings):
                await message.answer(
                    texts.op_intro(result.provider, settings.op_intro_sponsors),
                    reply_markup=keyboards.op_intro_keyboard(result.sponsors),
                )
            else:
                await message.answer(
