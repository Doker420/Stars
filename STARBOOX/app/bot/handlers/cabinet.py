...
from app.services import leaderboard, ledger, referrals, task_digest, withdrawals
...
async def profile_view(session: AsyncSession, user: User) -> tuple[str, InlineKeyboardMarkup]:
    ...
    return text, keyboards.profile_menu(user.task_digest_enabled)
...
async def digest_toggle(call: CallbackQuery, session: AsyncSession, db_user: User) -> None:
    """Flip the periodic task reminders from the profile screen."""
    await task_digest.set_opt_in(session, user=db_user, enabled=not db_user.task_digest_enabled)
    await safe_answer(call, texts.digest_toggled(db_user.task_digest_enabled))
    text, markup = await profile_view(session, db_user)
    await safe_edit(call.message, text, markup)


@router.callback_query(F.data == "menu:digest:off")
async def digest_off(call: CallbackQuery, session: AsyncSession, db_user: User) -> None:
    """One-tap opt-out straight from the reminder message."""
    await task_digest.set_opt_in(session, user=db_user, enabled=False)
    await safe_answer(call, texts.digest_toggled(False), alert=True)
