...
async def _tasks_view(
    session: AsyncSession,
    user: User,
    settings: Settings | None = None,
    op_gate: OpGate | None = None,
    bot: Bot | None = None,
):
    ...
    if settings is not None:
        if op_gate is not None and settings.partner_tasks_enabled:
            views = await partner_service.list_offers(
                session, user=user, gate=op_gate, settings=settings, bot=bot
            )
            partner_count = len(views)
        promo_count = len(await campaign_service.list_available(session, user=user, settings=settings))
    header = texts.tasks_header(
        len(done & {t.id for t in items}), len(items), partner_count, promo_count
    )
    return header, keyboards.tasks_keyboard(items, done, partner=partner_count, promo=promo_count)
...
async def menu_tasks(
    ...
    await state.clear()
    text, markup = await _tasks_view(session, db_user, settings, op_gate, bot)
    await safe_answer(call)
