...
from app.bot.handlers.promote import parse_campaign_payload
...
async def pre_checkout(query: PreCheckoutQuery, session: AsyncSession, db_user: User) -> None:
    ...
    campaign_id = parse_campaign_payload(query.invoice_payload)
    if campaign_id is not None:
        campaign = await session.get(Campaign, campaign_id)
        if campaign is None or campaign.owner_id != db_user.id:
            await query.answer(ok=False, error_message="Кампания не найдена")
            return
        if campaign.telegram_charge_id:
            await query.answer(ok=False, error_message="Кампания уже оплачена")
            return
        if query.currency != "XTR" or query.total_amount != campaign.xtr_price:
...
async def _fulfill_campaign(
    ...
    """Record the money, then flip the campaign to moderation/active (idempotent)."""
    campaign = await session.get(Campaign, campaign_id)
    if campaign is None or campaign.owner_id != db_user.id:
        log.error(
            "campaign_payment_without_campaign",
            payload=payment.invoice_payload,
            charge=payment.telegram_payment_charge_id,
        )
        await message.answer(
            "Оплата получена, но кампания не найдена. Напишите в /paysupport — мы разберёмся."
        )
        return
    ...
    if not created:
        log.info("campaign_payment_duplicate", charge=payment.telegram_payment_charge_id)
        return
    ...
    await message.answer(
        texts.campaign_card(campaign),
        reply_markup=keyboards.campaign_card(campaign),
    )
