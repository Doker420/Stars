...
def make_digest_sender(bot: Bot):
    """Deliver one periodic task reminder, with buttons into the relevant screens."""

    async def send(chat_id: int, payload) -> None:
        await bot.send_message(
            chat_id,
            texts.task_digest(payload.tasks, payload.campaigns, payload.partner),
            reply_markup=keyboards.digest_keyboard(
                payload.tasks, payload.campaigns, payload.partner
            ),
