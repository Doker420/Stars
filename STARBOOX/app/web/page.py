...
(async function () {
  ...
  if (!tg || !telegramInitData()) {
    show('⚠️', 'Нет данных Telegram', 'Вернитесь в личный чат с ботом и нажмите именно синюю кнопку «Подтвердить устройство».', 'Если кнопка открывает обычный браузер — проверьте WEB_PUBLIC_URL и домен Mini App в BotFather.');
    return;
  }
  tg.ready(); tg.expand();
...
<script>
const tg=window.Telegram&&window.Telegram.WebApp;let profile=null,rotation=0;if(tg){tg.ready();tg.expand();}function initData(){if(tg?.initData)return tg.initData;let q=new URLSearchParams(location.search),h=new URLSearchParams(location.hash.replace(/^#/,''));return q.get('tgWebAppData')||h.get('tgWebAppData')||''}
