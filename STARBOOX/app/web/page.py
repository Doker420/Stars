...
(async function () {
  ...
  if (!tg) {
    show('⚠️', 'Нет Telegram WebApp', 'Вернитесь в личный чат с ботом и нажмите синюю кнопку «Подтвердить устройство».', 'Не копируйте ссылку в обычный браузер.');
    return;
  }
  tg.ready(); tg.expand();
  // iOS/Android may populate initData a moment after the SDK object appears.
  for (let i = 0; i < 20 && !telegramInitData(); i++) {
    await new Promise((resolve) => setTimeout(resolve, 100));
  }
  if (!telegramInitData()) {
    show('⚠️', 'Telegram не передал подпись', 'Закройте окно и откройте его повторно синей кнопкой в личном чате с ботом.', 'Проверьте WEB_PUBLIC_URL и домен Mini App в BotFather.');
    return;
  }
  ...
  try {
    const res = await fetch('/api/device', {
      ...
      body: JSON.stringify({ initData: telegramInitData(), fingerprint, signals }),
    });
...
</style></head><body><main class="app"><section class="hero"><h1>⭐ STARBOOX</h1><div class="muted" id="name">Загрузка профиля…</div><div class="stats"><div class="stat"><b id="balance">0 ⭐</b><small>БАЛАНС</small></div><div class="stat"><b id="keys">0 🗝</b><small>КЛЮЧИ</small></div><div class="stat"><b id="spins">0 🎟</b><small>СПИНЫ</small></div></div><div class="links"><button onclick="share()">🤝 Пригласить</button><button onclick="reload()">🔄 Обновить</button></div></section><section class="card"><div class="title">🎡 РУЛЕТКА</div><div class="wheel" id="wheel">🎰</div><button id="spin" onclick="spin()">КРУТИТЬ</button></section><section class="card"><div class="title">🧰 КЕЙСЫ</div><div class="cases" id="cases"></div></section><section class="card"><div class="title">🛍 МАГАЗИН ЗА XTR</div><div class="cases"><div class="case"><div class="icon">🗝</div><b>5 ключей</b><button onclick="buy('keys_5')">15 XTR</button></div><div class="case"><div class="icon">🗝</div><b>15 ключей</b><button onclick="buy('keys_15')">39 XTR</button></div><div class="case"><div class="icon">💎</div><b>VIP 30 дней</b><button onclick="buy('vip_30')">99 XTR</button></div></div></section></main><div class="toast" id="toast"></div>
<script>
const tg=window.Telegram&&window.Telegram.WebApp;let profile=null,rotation=0;const el=id=>document.getElementById(id);if(tg){tg.ready();tg.expand();}function initData(){if(tg?.initData)return tg.initData;let q=new URLSearchParams(location.search),h=new URLSearchParams(location.hash.replace(/^#/,''));return q.get('tgWebAppData')||h.get('tgWebAppData')||''}
...
async function api(path,data={}){let r=await fetch(path,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...data,initData:initData()})});let j=await r.json();if(!r.ok)throw Error(j.error||'Ошибка');return j}
async function reload(){let nameEl=el('name');try{if(!initData())throw Error('Telegram не передал данные запуска. Откройте Mini App синей кнопкой в личном чате с ботом.');profile=await api('/api/game/profile');let u=profile.user;nameEl.textContent=u.name+(u.premium?' · 💎 Premium':'')+' · уровень '+u.level;el('balance').textContent=u.balance+' ⭐';el('keys').textContent=u.keys+' 🗝';el('spins').textContent=u.spins+' 🎟';el('cases').innerHTML=profile.cases.map(c=>`<div class="case"><div class="icon">${c.icon}</div><b>${c.title}</b>${c.free_price!==undefined?`<button onclick="openCase('${c.slug}','free')">БЕСПЛАТНО 1×/день</button>`:''}${c.key_price!==undefined?`<button onclick="openCase('${c.slug}','key')">🗝 ${c.key_price}</button>`:''}${c.stars_price!==undefined?`<button onclick="openCase('${c.slug}','stars')">⭐ ${c.stars_price}</button>`:''}</div>`).join('')}catch(e){nameEl.textContent='Ошибка загрузки профиля';toast(e.message)}}
async function spin(){let btn=el('spin'),wheelEl=el('wheel');try{btn.disabled=true;let j=await api('/api/game/spin',{requestId:rid()});rotation+=1440+Math.floor(Math.random()*360);wheelEl.style.transform=`rotate(${rotation}deg)`;setTimeout(()=>toast('Вы выиграли: '+j.reward.amount+' '+j.reward.kind),2100);setTimeout(reload,2300)}catch(e){toast(e.message)}finally{setTimeout(()=>btn.disabled=false,2300)}}
...
async function buy(product){try{let j=await api('/api/game/invoice',{product});if(!tg?.openInvoice)throw Error('Откройте Mini App внутри Telegram');tg.openInvoice(j.invoice,s=>{if(s==='paid'){toast('✅ Покупка оплачена');setTimeout(reload,1200)}})}catch(e){toast(e.message)}}
