...
def ensure_action_cooldown(user: User, settings: Settings) -> None:
    """Rate-limit *rewarded* actions.

    Measured against ``last_claim_at`` (set by :func:`bump_activity` after a payout),
    never against ``last_action_at`` — the user middleware refreshes that on every
    single update, so using it here would reject every claim forever.
    """
    if not user.last_claim_at or settings.claim_cooldown_seconds <= 0:
        return
    ...
    if elapsed < wait:
        left = max(int((wait - elapsed).total_seconds() + 0.999), 1)
        raise CooldownActive(f"Слишком часто. Подождите {left} с.")


async def bump_activity(session: AsyncSession, user: User, points: int = 1) -> None:
    """Record a rewarded action: bumps activity and starts the claim cooldown."""
