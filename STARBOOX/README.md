...
- **Задания партнёров**: офферы из API всех 5 сервисов монетизации публикуются как оплачиваемые задания; выполнение подтверждается самим партнёром, награда зачисляется мгновенно и только один раз
- **Продвижение за Stars**: пользователь размещает свой канал / бота / пост / произвольное задание, платит XTR за нужное число выполнений и следит за прогрессом (пауза, возобновление)
...
| 📣 Продвижение | очередь модерации платных кампаний, карточка с заказчиком и объёмом, одобрить / отклонить с причиной и **автовозвратом Stars**, отчёт по партнёрским заданиям (выполнено / выплачено по каждому сервису) |
...
| Партнёрские задания | `PARTNER_TASKS_ENABLED`, `PARTNER_TASKS_LIMIT`, `PARTNER_REWARD_CHANNEL/BOT/BOOST/RESOURCE` |
| Продвижение за Stars | `PROMO_CAMPAIGNS_ENABLED`, `CAMPAIGN_MODERATION`, `CAMPAIGN_MIN_TARGET`, `CAMPAIGN_MAX_TARGET`, `CAMPAIGN_REWARD_CHANNEL/BOT/POST/CUSTOM`, `CAMPAIGN_XTR_PER_REWARD`, `CAMPAIGN_SERVICE_FEE_PERCENT`, `CAMPAIGN_MAX_ACTIVE_PER_USER` |
| Логи | `LOG_LEVEL`, `LOG_JSON` |

`OP_INTRO_ENABLED` / `OP_INTRO_SPONSORS` / `OP_INTRO_GRACE_SEC` — мягкий первый вход (см. «Каскад ОП»).

`TASK_DIGEST_*` — периодические напоминания о заданиях (см. ниже).

`STARTUP_API_TIMEOUT_SEC` — предел ожидания Telegram API на старте (по умолчанию 15 с). Без него
недоступная сеть превращала запуск в молчаливое зависание: таймаут самой aiogram-сессии — 60 с,
плюс повторы.

### Приоритет партнёрских заданий

Партнёрские задания — источник дохода проекта, поэтому в интерфейсе они везде идут первыми:
первая кнопка главного меню (во всю ширину, со счётчиком доступных офферов), первая строка
экрана «Задания» и его заголовка, первая строка и первая кнопка в напоминании. Порядок закреплён
тестами (`tests/test_partner_priority.py`), чтобы случайный рефактор не опустил их вниз.

Счётчик в главном меню берётся из последнего реального запроса офферов (кэш в памяти), поэтому
бейдж не добавляет обращений к провайдерам на каждый `/start`.

### Напоминания о заданиях

Фоновый воркер (`app/services/task_digest.py`) раз в несколько минут просыпается и рассылает
короткий дайджест тем, у кого есть невыполненные задания или кампании других пользователей.
Гарантии, заложенные в дизайн:

* не чаще одного сообщения в `TASK_DIGEST_INTERVAL_HOURS` — отметка хранится в БД
  (`users.last_task_digest_at`), поэтому перезапуск бота не вызывает повторную волну;
* молчание в тихие часы `TASK_DIGEST_QUIET_FROM..TASK_DIGEST_QUIET_TO` (время сервера);
* если предлагать нечего — сообщение не отправляется вовсе;
* пользователь отключает напоминания кнопкой в профиле или прямо из сообщения
  («🔕 Не напоминать»), флаг `users.task_digest_enabled` учитывается в самом SQL-запросе;
* заблокировавшие бота помечаются автоматически и выпадают из аудитории.

Партнёрские офферы **входят** в дайджест и стоят первыми: это монетизируемый инвентарь, ради
которого напоминание и существует. Их подсчёт стоит одного запроса к провайдерам на пользователя
в пачке — если упрётесь в лимиты API, отключите `TASK_DIGEST_INCLUDE_PARTNER`, остальные источники
продолжат работать. Сбой провайдера никогда не отменяет напоминание: партнёрских просто будет 0.

### Если бот «завис на запуске»

Процесс намеренно не падает, когда `BOT_TOKEN` пустой или неверный, — иначе панель хостинга уходит
в бесконечный рестарт-луп. Он пишет `telegram_unauthorized` (или `idle_after_placeholder_token`) и
ждёт. Это **не** зависание: по SIGTERM (кнопка Stop/Restart в панели) он корректно завершится и
напишет `stopped_by_signal`. Если логи обрываются на `starting` — смотрите `get_me_timeout`:
значит, до `api.telegram.org` нет доступа.

`CLAIM_COOLDOWN_SECONDS` — антиспам между **начислениями** (ежедневка, задания, промокоды,
партнёрские задания, кампании). Отсчитывается от `users.last_claim_at`, то есть от последней
*награды*, а не от последнего действия: обычная навигация по меню кулдаун не продлевает.
...
## Задания партнёров (монетизация по API)

Каскад ОП — это **шлагбаум**: он не пускает пользователя дальше, пока не выполнены обязательные
подписки. Те же API дополнительно отдают **необязательные офферы**, и бот публикует их как
оплачиваемые задания в разделе «🤝 Задания партнёров».

| Провайдер | Метод офферов | Подтверждение |
| --- | --- | --- |
| Flyer | `POST /get_tasks` (ключ типа `tasks`) | `POST /check_task` по `signature`, иначе перечитывает список |
| SubGram | `POST /get-sponsors` с `action=newtask` | `POST /get-user-subscriptions` по ссылке |
| BotoHub | `POST /get-tasks-extended` | повторный запрос, читается флаг `completed` |
| PiarFlow | `POST /sponsors` | `POST /sponsors/check` по ссылке |
| TGrass | `POST /offers` | повторный запрос, читается `subscribed` |
| Trafsly | `POST /api/v1/get-sponsors` | `POST /api/v1/confirm-subscription` по `ads_id` |

Как это работает:

1. Экран заданий опрашивает все включённые провайдеры и показывает офферы с ценой из настроек
   (`PARTNER_REWARD_*` — ставка за тип: канал / бот / буст / ссылка).
2. Нажатие на задание создаёт запись `partner_task_claims` со статусом `pending` — цена и ссылка
   фиксируются, пока пользователь ходит к спонсору.
3. «Проверить» повторно спрашивает партнёра. Деньги начисляются **только при явном `да`**;
   «ещё нет» и таймаут API ничего не списывают и не платят.
4. Уникальный индекс `(user_id, provider, external_id)` делает повторную выплату невозможной —
   даже при одновременных нажатиях. Уже оплаченные офферы исчезают из списка.

Провайдеры опрашиваются best-effort: упавший API просто не даёт офферов и не ломает экран.
Ставки и лимит выдачи меняются на лету в **Админка → ⚙️ Настройки**, весь раздел выключается
флагом `PARTNER_TASKS_ENABLED`.

## Продвижение за Telegram Stars

Пользователь может купить размещение внутри бота — раздел «📣 Продвижение».

Форматы: **📢 канал** (подписка), **🤖 бот** (запуск), **📝 пост** (просмотр), **🔗 своё задание**.

Модель оплаты — CPA, платят за выполнения:

```
цена XTR = количество × ставка(формат) × CAMPAIGN_XTR_PER_REWARD × (100 + комиссия%) / 100
```
...
draft → awaiting_payment → (moderation) → active → done
                                ↓
                             rejected   ← Stars возвращаются автоматически
```

* Мастер из 5 шагов заканчивается счётом в XTR прямо в Telegram; оплата идемпотентна по
  `telegram_payment_charge_id` (повторная доставка `successful_payment` ничего не дублирует).
* При `CAMPAIGN_MODERATION=true` кампания уходит в очередь админам; отклонение возвращает оплату
  через `refundStarPayment`. Флаг выключается — публикация мгновенная.
* Подписка на канал проверяется через `getChatMember` (бот должен быть админом канала), пост и
  «своё задание» — на доверии, под общим антиспам-кулдауном.
* Бюджет нельзя перерасходовать: инкремент `done_count` идёт условным `UPDATE … WHERE done_count <
  target_count`, поэтому параллельные выполнения не уводят счётчик за цель. По достижении цели
  кампания автоматически становится `done`, заказчик получает уведомление.
* Свою кампанию выполнить нельзя, одно выполнение на пользователя (`uq_campaign_claim`), лимит
  активных кампаний на пользователя — `CAMPAIGN_MAX_ACTIVE_PER_USER`.
...
Провайдер, который сказал «не пущу», но не дал ни одной ссылки (и не прислал свой блок ОП сам),
пропускается: иначе пользователь упирался бы в сообщение без кнопок. Статусы, которые провайдер
сам не смог проверить (`notgetted` у SubGram), и типы ресурсов без проверки подписки
(`resource`, `smart_link`) не блокируют вход — иначе «Я подписался» не срабатывает никогда.

### Мягкий первый вход

На первом `/start` бот показывает не весь каскад, а только `OP_INTRO_SPONSORS` спонсоров (по
умолчанию 2) и сразу открывает меню на `OP_INTRO_GRACE_SEC` секунд (по умолчанию час). Пользователь
успевает увидеть функционал до того, как его просят подписываться, а пропущенные спонсоры
возвращаются к нему уже как **оплачиваемые** задания партнёров. После окончания окна действует
обычный полный каскад. Выключается переключателем `OP_INTRO_ENABLED` (и в админке).

Задания партнёров собираются со **всех** провайдеров сразу: лимит `PARTNER_TASKS_LIMIT` делится
поровну, поэтому провайдер с большим инвентарём не вытесняет остальных, а неиспользованные слоты
перераспределяются. Офферы, которые провайдер уже считает выполненными, в список не попадают —
это обязательные подписки, за которые платить не за что.
...
app/
  ...
  bot/
    ...
    handlers/            # пользовательские роутеры (start, commands, cabinet, earn, partner,
                         # promote, withdraw, promo, payments, system)
  ...
  services/              # ledger, users, referrals, daily, tasks, partner_tasks, campaigns,
                         # boosts, payments, economy, withdrawals, promo, access, audit,
                         # app_settings, broadcasts, stats, leaderboard, export, antifraud, events
